package me.jiangcai.dating.core;

/**
 * @author CJ
 */
public class Login {
    public static final String Role_User_Value = "USER";
    public static final String Role_Edit_Value = "EDIT";
    public static final String Role_Agent_Value = "AGENT";
    public static final String Role_Order_Value = "ORDER";
    public static final String Role_Finance_Value = "FINANCE";
    public static final String Role_Loan_Value = "LOAN";
    public static final String Role_Project_Loan_Value = "PROJECT_LOAN";
    public static final String Role_Grant_Value = "GRANT";
    public static final String Role_Manage_Value = "MANAGE";

    /**
     * 可读取商品
     */
    public static final String Role_Sale_Goods_Read_Value = "SALE_GOODS_READ";
    /**
     * 可管理商品
     */
    public static final String Role_Sale_Goods_Value = "SALE_GOODS";
    /**
     * 可管理库存
     */
    public static final String Role_Sale_Stock_Value = "SALE_STOCK";
    /**
     * 可管理订单
     */
    public static final String Role_Sale_Trade_Value = "SALE_TRADE";
}
